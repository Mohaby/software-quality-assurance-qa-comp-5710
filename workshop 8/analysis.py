...
def checkFlow(data, code):
    ...
    if os.path.exists(code):
        # Initialize an empty dictionary to store the flow of variables
        flow = {}
        # Parse the full code into an abstract syntax tree
        full_tree = ast.parse(open(code).read())
        
        # Iterate through variable assignments in the code and populate the flow dictionary
        for var in getVariables(full_tree, 'VAR_ASSIGNMENT'):
            # If the assigned value is a number, add it as a key to the flow dictionary
            if isinstance(var[1], int):
                flow[var[1]] = var[0]
            # If the assigned value is a string without commas, add it as a key to the flow dictionary
            elif isinstance(var[1], str) and "," not in var[1]:
                flow[var[1]] = var[0]
            # If the assigned value is a string with commas, split and add each value as a key to the flow dictionary
            elif isinstance(var[1], str) and "," in var[1]:
                for v in var[1].split(','):
                    flow[v] = var[0]
        
        # Initialize a dictionary to store the trace of function calls and assignments
        trace = {}
        # Extract function definitions and variables from the code
        funcDefList, funcVarList = getFunctionDefinitions(code)
        # Add function calls to the trace dictionary
        for declaration in funcDefList:
            trace[declaration[2][-1]] = declaration[1]
        # Add function variable assignments to the trace dictionary
        for call in getFunctionAssignments(full_tree):
            trace[call[2]] = trace[call[3][-1]]
                
        # Merge flow and trace dictionaries
        trace = {**trace, **flow}
        
        # Trace the data flow until it reaches a final state
        while data in trace:
            # Print the current data
            print(data, end="->")
            # Update the data according to the flow
            data = trace[data]
